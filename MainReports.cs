...
namespace CMPT291_GROUP_PROJECT
{
    public partial class MainReports : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if ((dflag != 0) && (mflag != 0) && (yflag != 0) ) 
            { string dt = $"{yr}\\{mth}\\{dy}";
                if (renRB.Checked)
                {
                    myCommand.CommandText = $"select count(*) as Counts from Orders where DateFrom = {dt}";
                ...
                else if (mvRB.Checked)
                {
                    myCommand.CommandText = $"select  M.MovieID,M.Title, T.Counts from Movies as M ,(select MovieID, count(*) as Counts from Orders" +
                        $" where DateFrom = {dt} group by MovieID) " +
                    ...
                    while (myReader.Read())
                    {
                        dailyG.Rows.Add(myReader["MovieID"], myReader["Title"], myReader["Counts"].ToString());
                    }
                }
        ...
        private void gen2BT_Click(object sender, EventArgs e)
        {
            ...
            if ((mflag != 0) && (yflag != 0))
            {
                ...
                {
                    myCommand.CommandText = $"select T.CustomerID, Customer.FName, T.Sales from Customer as C,(select CustomerID, count(*) as Sales from Orders" +
